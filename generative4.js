...
function animate() {
  ...
  if ((change % 9 == 0) && (beatVals.length > 9)) {
    ...
    if (beatAvg > beatThresh) {
      ...
      if (volAvg <= divider) {
        console.log("low");
        drawFlower(volAvg, 0x997825, 'low');
      } else if (volAvg <= divider*2 && volAvg >= divider) {
        console.log("medium");
        drawFlower(volAvg, 0x000000, 'med');
      } else if (volAvg >= divider*2 && volAvg <= BIN_COUNT) {
        console.log("high");
        drawFlower(volAvg, 0x15776E, 'high');
      }
...
function drawFlower(volAvg,petalColor,frequency) {
  var material = new THREE.MeshBasicMaterial({
      ...
      switch(frequency) {
        case 'low': 
          petalMesh.position.z = -1000;
          break;
        case 'med':
          petalMesh.position.z = -500;
          break;
        case 'high':
          petalMesh.position.z = 0;
          break;
        default:
